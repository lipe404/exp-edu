...
const ScrollManager = {
    ...
    setupScrollAnimations() {
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (entry.target.hasAttribute('data-counter')) {
                        const finalValue = parseInt(entry.target.getAttribute('data-counter'));
        ...
        document.querySelectorAll('.animate-fade-in-up, [data-counter]').forEach(el => {
            observer.observe(el);
        });
    }
};
...
const TestimonialCarousel = {
    ...
    goToPrevious() {
        const totalSlides = this.indicators.length;
        const prevIndex = AppState.currentTestimonial === 0 ? totalSlides - 1 : AppState.currentTestimonial - 1;
...
const ProfitabilityCalculator = {
    ...
    updateStudentRange() {
        ...
        switch (type) {
            case 'small':
                min = 20; max = 50; defaultValue = 35;
                break;
            case 'medium':
                min = 50; max = 150; defaultValue = 100;
                break;
            case 'large':
                min = 150; max = 300; defaultValue = 200;
                break;
            default:
                min = 20; max = 300; defaultValue = 50;
        }
    ...
    calculate() {
        ...
        // Atualizar display
        this.grossRevenue.textContent = `R$ ${grossRevenue.toLocaleString('pt-BR')}`;
        this.commission.textContent = `R$ ${commission.toLocaleString('pt-BR')}`;
        this.operationalCosts.textContent = `R$ ${operationalCosts.toLocaleString('pt-BR')}`;
        this.netProfit.textContent = `R$ ${netProfit.toLocaleString('pt-BR')}`;
        this.annualProfit.textContent = `R$ ${annualProfit.toLocaleString('pt-BR')}`;
...
const FormManager = {
    ...
    validateForm() {
        ...
        if (!termsCheckbox.checked) {
            this.showFieldError(termsCheckbox, 'Você deve aceitar os termos e condições');
            isValid = false;
        }
    ...
    async sendFormData(data) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                if (Math.random() > 0.1) {
                    resolve({ success: true, message: 'Formulário enviado com sucesso!' });
                } else {
                    resolve({ success: false, message: 'Erro temporário. Tente novamente.' });
                }
            }, 2000);
        });
    ...
    showSuccessMessage() {
        ...
        setTimeout(() => {
            window.open('https://wa.me/5531999998888?text=Olá! Acabei de preencher o formulário de parceria no site. Gostaria de mais informações.', '_blank');
        }, 2000);
    },

    showErrorMessage(message) {
        const errorDiv = document.createElement('div');
        errorDiv.className = 'fixed top-4 right-4 bg-red-500 text-white p-4 rounded-lg shadow-lg z-50 animate-fade-in';
        errorDiv.innerHTML = `
            <div class="flex items-center">
...
const AnalyticsManager = {
    ...
    setupClickTracking() {
        // Tracking de CTAs
        document.querySelectorAll('a[href="#formulario-parceria"]').forEach(cta => {
